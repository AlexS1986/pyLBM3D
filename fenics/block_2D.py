...
# Stress function
def sig(u):
    return lmbda*dlf.div(u)*dlf.Identity(2) + 2*mu*eps(u)


# --------------------
# Geometry
# --------------------
mesh = dlf.RectangleMesh(dlf.Point(-1.0, -1.0), dlf.Point(1.0, 1.0), n, n)
...
tDt = dlf.Expression(("0"," t < 2.0 ? (t/2.0)*t0 : t0"), t0 = t0, t=0.0, degree=1)
tDb = dlf.Expression(("0"," t < 2.0 ? -(t/2.0)*t0 : -t0"), t0 = t0, t=0.0, degree=1)
...
file = dlf.XDMFFile("block_2D_out.xdmf")  # XDMF file
...
for i in range(150):
    ...
    if output:
        file.write(u, t)
        print("p0", t, u(0.025,0.975)[1],  u(0.475,0.975)[1],  u(0.975,0.975)[1])
